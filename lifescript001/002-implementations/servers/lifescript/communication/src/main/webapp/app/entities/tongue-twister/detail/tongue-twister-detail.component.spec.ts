...
describe('TongueTwister Management Detail Component', () => {
      ...
      providers: [
        provideRouter(
          [
            {
              path: '**',
              loadComponent: () => import('./tongue-twister-detail.component').then(m => m.TongueTwisterDetailComponent),
              resolve: { tongueTwister: () => of({ id: 1199 }) },
            },
          ],
  ...
  describe('OnInit', () => {
    it('Should load tongueTwister on init', async () => {
      ...
      // THEN
      expect(instance.tongueTwister()).toEqual(expect.objectContaining({ id: 1199 }));
    });
  });
