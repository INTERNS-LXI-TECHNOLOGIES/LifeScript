...
type RestOf<T extends IAlert | NewAlert> = Omit<T, 'timestamp'> & {
...
export class AlertService {
  ...
  protected convertDateFromClient<T extends IAlert | NewAlert | PartialUpdateAlert>(alert: T): RestOf<T> {
    return {
      ...alert,
      timestamp: alert.timestamp?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restAlert: RestAlert): IAlert {
    return {
      ...restAlert,
      timestamp: restAlert.timestamp ? dayjs(restAlert.timestamp) : undefined,
    };
  }
